...
class LiberoOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
        # Only return the first 7 dims for actions.
        # Cast language_prefix_logits to float32 to avoid the bfloat16 issue.
        return {
            "actions": np.asarray(data["actions"][:, :7]),
            # "language_prefix_logits": np.asarray(data["language_prefix_logits"]).astype(np.float32),
            # "token_probs": np.asarray(data["token_probs"]).astype(np.float32),
        }
