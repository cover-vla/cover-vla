...
from video_utils import save_rollout_video
...
def eval_libero(args: Args) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        print('task_description 1', task_description)
        # Start episodes
        task_episodes, task_successes = 0, 0
        original_task_description = str(task_description)
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            print('original_task_description', original_task_description)
            if args.lang_transform != 'no_transform' and args.lang_transform != 'interpolation':
                print('before task_description', original_task_description)
                task_description = lang_transform.transform(original_task_description, args.lang_transform)
                print('after task_description', task_description)
        
            elif args.lang_transform == 'interpolation':
                task_description_list = original_task_description.split()
                print('task_description_list', task_description_list)
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    if args.lang_transform == 'interpolation':
                        if count_interp == 0:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Force cleanup of OpenGL resources
        import OpenGL.EGL as EGL
        try:
            EGL.eglTerminate(EGL.eglGetCurrentDisplay())
        except:
            pass
