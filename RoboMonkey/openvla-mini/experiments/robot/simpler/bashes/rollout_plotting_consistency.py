...
def analyze_rollouts(rollout_dir="./rollouts_oracle"):
    ...
    if results:
        ...
        create_consistency_time_series_plots(time_series_data, plots_dir)  # Add consistency plots
        create_task_consistency_time_series_plots(rollout_dir)  # Add per-task consistency plots
        create_language_instruction_distance_plots(rollout_dir, embedding_type='text')  # Add language instruction distance analysis
        create_selected_phrases_bar_plots(rollout_dir)  # Add selected phrases analysis
...
def process_folder(folder_path, folder_name):
    ...
    # For consistency score analysis
    success_consistency_series = []
    failure_consistency_series = []
    ...
    for pkl_file in pkl_files:
        ...
        try:
            with open(pkl_file, "rb") as f:
                ...
                # Extract consistency scores if available
                consistency_scores = data.get("all_consistency_scores", None)
        ...
        if is_success:
            ...
            success_consistency_series.append(consistency_scores) # Store consistency scores
        else:
            ...
            failure_consistency_series.append(consistency_scores) # Store consistency scores
    ...
    folder_time_series = {
        ...
        "success_consistency_series": success_consistency_series, # Add consistency scores here
        "failure_consistency_series": failure_consistency_series  # Add consistency scores here
    }
...
def create_combined_time_series_plot(time_series_data, plots_dir):
    ...
    if "oracle" in plots_dir:
        fig.supylabel('Average Oracle Score / Action Norm / Consistency Score', x=0.01) # Add shared Y label
    else:
        fig.supylabel('Average CLIP Score / Action Norm / Consistency Score', x=0.01) # Add shared Y label
...
def plot_time_series(ax, folder, data, is_subplot=True):
    """Plot time series data (scores, consistency scores, and action norms) for a single folder on the given axes."""
    ...
    success_consistency_series = data.get("success_consistency_series", [])
    failure_consistency_series = data.get("failure_consistency_series", [])
    ...
    success_consistency_by_timestep = [[] for _ in range(max_len)] # Consistency scores
    failure_consistency_by_timestep = [[] for _ in range(max_len)] # Consistency scores

    for series, action_series, consistency_series in zip(success_series, success_action_series, success_consistency_series):
        for t, (score, action) in enumerate(zip(series, action_series)):
             ...
             if not np.isnan(score_float): # Only store non-NaN scores and corresponding actions
                ...
                if consistency_series is not None and t < len(consistency_series):
                    consistency_score = consistency_series[t]
                    if consistency_score is not None and not np.isnan(consistency_score):
                        success_consistency_by_timestep[t].append(consistency_score)

    for series, action_series, consistency_series in zip(failure_series, failure_action_series, failure_consistency_series):
        for t, (score, action) in enumerate(zip(series, action_series)):
             score_float = float(score) if score is not None else np.nan # Convert to float, handle None
             if score_float == 0: score_float = np.nan # Replace 0 with NaN
             if not np.isnan(score_float): # Only store non-NaN scores and corresponding actions
                 ...
                 if consistency_series is not None and t < len(consistency_series):
                     consistency_score = consistency_series[t]
                     if consistency_score is not None and not np.isnan(consistency_score):
                         failure_consistency_by_timestep[t].append(consistency_score)
    ...
    avg_success_consistency = np.full(max_len, np.nan)
    avg_failure_consistency = np.full(max_len, np.nan)
    std_success_consistency = np.full(max_len, np.nan)
    std_failure_consistency = np.full(max_len, np.nan)
    ...
    for t in range(max_len):
        ...
        consistency_t_success = success_consistency_by_timestep[t]
        if scores_t_success: # If list is not empty (already filtered for NaNs)
            ...
            # Calculate consistency score statistics
            if consistency_t_success:
                avg_success_consistency[t] = np.nanmean(consistency_t_success)
                if len(consistency_t_success) > 1:
                    std_success_consistency[t] = np.nanstd(consistency_t_success)

        # Failure stats
        scores_t_failure = failure_scores_by_timestep[t]
        actions_t_failure = failure_actions_by_timestep[t]
        consistency_t_failure = failure_consistency_by_timestep[t]
        if scores_t_failure: # If list is not empty (already filtered for NaNs)
            ...
            # Calculate consistency score statistics
            if consistency_t_failure:
                avg_failure_consistency[t] = np.nanmean(consistency_t_failure)
                if len(consistency_t_failure) > 1:
                    std_failure_consistency[t] = np.nanstd(consistency_t_failure)

    # --- Plotting on the provided axes ---
    ax1 = ax # Use the primary axis for CLIP scores
...
def plot_consistency_time_series(ax, folder, data, is_subplot=True):
    """Plot time series data for consistency scores only."""
    success_consistency_series = data.get("success_consistency_series", [])
    failure_consistency_series = data.get("failure_consistency_series", [])
    
    # Find the maximum length across all trajectories for this condition
    # Filter out None values before calculating lengths
    valid_success_series = [series for series in success_consistency_series if series is not None]
    valid_failure_series = [series for series in failure_consistency_series if series is not None]
    
    max_len_success = max([len(series) for series in valid_success_series]) if valid_success_series else 0
    max_len_failure = max([len(series) for series in valid_failure_series]) if valid_failure_series else 0
    max_len = max(max_len_success, max_len_failure)

    if max_len == 0: # No data to plot for this folder
         ax.set_title(f'{folder} (No consistency data)')
         ax.text(0.5, 0.5, 'No consistency data available', horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
         return

    # --- Accumulate consistency data per timestep ---
    success_consistency_by_timestep = [[] for _ in range(max_len)]
    failure_consistency_by_timestep = [[] for _ in range(max_len)]

    for series in valid_success_series:
        for t, consistency_score in enumerate(series):
            if t < max_len and consistency_score is not None and not np.isnan(consistency_score):
                success_consistency_by_timestep[t].append(consistency_score)

    for series in valid_failure_series:
        for t, consistency_score in enumerate(series):
            if t < max_len and consistency_score is not None and not np.isnan(consistency_score):
                failure_consistency_by_timestep[t].append(consistency_score)
    ...
    valid_success_counts = np.zeros(max_len, dtype=int)
    valid_failure_counts = np.zeros(max_len, dtype=int)

    for t in range(max_len):
        # Success consistency stats
        consistency_t_success = success_consistency_by_timestep[t]
        if consistency_t_success:
            valid_success_counts[t] = len(consistency_t_success)
            avg_success_consistency[t] = np.nanmean(consistency_t_success)
            if len(consistency_t_success) > 1:
                std_success_consistency[t] = np.nanstd(consistency_t_success)

        # Failure consistency stats
        consistency_t_failure = failure_consistency_by_timestep[t]
        if consistency_t_failure:
            valid_failure_counts[t] = len(consistency_t_failure)
            avg_failure_consistency[t] = np.nanmean(consistency_t_failure)
            if len(consistency_t_failure) > 1:
                std_failure_consistency[t] = np.nanstd(consistency_t_failure)
    ...
    if np.any(~np.isnan(avg_success_consistency)):
        ax.plot(timesteps, avg_success_consistency, 'g-', label=f'Success (n={len(valid_success_series)})', linewidth=2)
        ax.fill_between(timesteps, avg_success_consistency - std_success_consistency, avg_success_consistency + std_success_consistency,
                            color='g', alpha=0.2, where=~np.isnan(avg_success_consistency))

    if np.any(~np.isnan(avg_failure_consistency)):
        ax.plot(timesteps, avg_failure_consistency, 'r-', label=f'Failure (n={len(valid_failure_series)})', linewidth=2)
        ax.fill_between(timesteps, avg_failure_consistency - std_failure_consistency, avg_failure_consistency + std_failure_consistency,
                            color='r', alpha=0.2, where=~np.isnan(avg_failure_consistency))
    ...
    legend_loc = 'upper left' if not is_subplot else 'best'
    bbox = (1.05, 1) if not is_subplot else None
    ax.legend(loc=legend_loc, bbox_to_anchor=bbox, fontsize='small',
               title=f'Max Valid Samples/Step:\nSuccess: {np.max(valid_success_counts) if valid_success_counts.size > 0 else 0}, Failure: {np.max(valid_failure_counts) if valid_failure_counts.size > 0 else 0}')

    success_rate = len(valid_success_series) / (len(valid_success_series) + len(valid_failure_series)) if (len(valid_success_series) + len(valid_failure_series)) > 0 else 0
    ax.set_title(f'{folder} Consistency Scores (Success Rate: {success_rate:.1%})', fontsize=10)

    if not is_subplot:
        ax.set_xlabel('Timestep')

def create_consistency_time_series_plots(time_series_data, plots_dir):
    """
    Create time series plots showing consistency scores only.
    """
    if not time_series_data: return # Skip if no data

    # Check if any folder has consistency data before creating plots
    has_consistency_data = False
    for folder, data in time_series_data.items():
        success_consistency = data.get("success_consistency_series", [])
        failure_consistency = data.get("failure_consistency_series", [])
        if success_consistency or failure_consistency:
            # Check if any series actually has data (filter out None values)
            valid_series = [series for series in success_consistency + failure_consistency if series is not None]
            if valid_series:
                has_consistency_data = True
                break

    if not has_consistency_data:
        print("No consistency data found in any folder. Skipping consistency plots.")
        return

    # Create combined plot with subplots
    create_combined_consistency_plot(time_series_data, plots_dir)

    # Create individual plots for each condition type
    create_individual_consistency_plots(time_series_data, plots_dir)

def create_combined_consistency_plot(time_series_data, plots_dir):
    """Create a combined plot with subplots for consistency scores."""
    n_folders = len(time_series_data)
    if n_folders == 0: return
    ...
    for i, (folder, data) in enumerate(time_series_data.items()):
        plot_consistency_time_series(axes_flat[i], folder, data, is_subplot=True)
    ...
    # Set common x-axis label (centered at the bottom)
    fig.supxlabel('Timestep', y=0.02) # Adjust y position if needed
    fig.supylabel('Average Consistency Score', x=0.01) # Add shared Y label
    fig.suptitle("Consistency Score Time Series Analysis per Condition", fontsize=16, y=0.99) # Add overall title

    plt.tight_layout(rect=(0.03, 0.03, 1, 0.97)) # Adjust layout to prevent overlap

    # Save the figure
    save_path = os.path.join(plots_dir, "consistency_time_series_analysis_combined.png")
    plt.savefig(save_path)
    print(f"Saved combined consistency time series plot to {save_path}")
    plt.close()

def create_individual_consistency_plots(time_series_data, plots_dir):
    """Create individual plots for each condition type showing consistency scores."""
    for folder, data in time_series_data.items():
        # Skip if no consistency data
        success_consistency = data.get("success_consistency_series", [])
        failure_consistency = data.get("failure_consistency_series", [])
        
        # Check if there's any actual consistency data (filter out None values)
        valid_series = [series for series in success_consistency + failure_consistency if series is not None]
        has_data = len(valid_series) > 0
        ...
        fig = plt.figure(figsize=(10, 6))
        ax = fig.gca() # Get current axis
        plot_consistency_time_series(ax, folder, data, is_subplot=False) # Use the single plotting function

        plt.tight_layout()
        save_path = os.path.join(plots_dir, f"{safe_folder_name}_consistency_time_series.png")
        plt.savefig(save_path)
        print(f"Saved individual consistency time series plot for {folder} to {save_path}")
        plt.close()
...
def create_task_success_rate_plots(rollout_dir):
    """
    For each unique task (language instruction), plot the success rate across rephrase counts,
    drawing separate lines for consistency variants when folders are named like:
      - rephrase_{N}_consistency_{True|False}

    X-axis: number of samples (rephrase N)
    ...
    Special handling:
    - no_transform_1: plotted as red dashed horizontal line (oracle policy baseline)
    - robomonkey: plotted as orange dashed horizontal line (robomonkey baseline)
    - rephrase_N[_consistency_X]: plotted as separate lines per consistency flag
    """
    import matplotlib.pyplot as plt
    import os
    import re
    ...
    # Aggregate:
    # task -> "consistency=True/False/None" -> rephrase_num -> [success/failure]
    # Note: "None" bucket collects folders without an explicit consistency flag (e.g., plain rephrase_N)
    task_consistency_rephrase_results = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))

    for trans in transformation_folders:
        # Determine rephrase number and optional consistency flag for this transformation
        ...
        if trans == "no_transform_1":
            ...
            # Support both patterns: rephrase_N and rephrase_N_consistency_{True|False}
            rephrase_match = re.search(r'rephrase_(\d+)', trans)
            if not rephrase_match:
                continue
            rephrase_num = int(rephrase_match.group(1))
            consistency_match = re.search(r'consistency_(True|False)', trans)
            if consistency_match:
                consistency_flag = consistency_match.group(1)
        ...
        if trans == "robomonkey":
            ...
            for mp4_file in mp4_files:
                ...
                if not success_match:
                    continue
                is_success = success_match.group(1) == 'True'
                # Store in a flat structure for baselines; handled later during plotting
                task_consistency_rephrase_results[task]["baseline_robomonkey"][rephrase_num].append(is_success)
        else:
            ...
            for pkl_file in pkl_files:
                ...
                if rephrase_num == 0:
                    # no_transform_1 baseline
                    task_consistency_rephrase_results[task]["baseline_no_transform"][rephrase_num].append(is_success)
                else:
                    bucket = f"consistency_{consistency_flag}" if consistency_flag is not None else "consistency_None"
                    task_consistency_rephrase_results[task][bucket][rephrase_num].append(is_success)
    ...
    for task, consistency_map in task_consistency_rephrase_results.items():
        # Gather series for consistency variants
        series = []  # list of (label, x_list, y_list, n_list, color)

        # Define colors for consistency variants
        color_for_bucket = {
            "consistency_True": 'mediumseagreen',
            "consistency_False": 'royalblue',
            "consistency_None": 'purple',
        }

        # Add rephrase series per consistency
        for bucket_key, rephrase_dict in consistency_map.items():
            if bucket_key.startswith("baseline_"):
                continue
            rephrase_nums = sorted([k for k in rephrase_dict.keys() if k > 0])
            if not rephrase_nums:
                continue
            rates = []
            counts = []
            for r in rephrase_nums:
                values = rephrase_dict[r]
                if values:
                    ...
                    rates.append(0)
                    counts.append(0)
            label = bucket_key.replace("_", "=") if bucket_key != "consistency_None" else "rephrases"
            color = color_for_bucket.get(bucket_key, 'gray')
            series.append((label, rephrase_nums, rates, counts, color))

        # Compute baselines (horizontal lines)
        oracle_rate = None
        if "baseline_no_transform" in consistency_map and 0 in consistency_map["baseline_no_transform"]:
            values = consistency_map["baseline_no_transform"][0]
            if values:
                oracle_rate = sum(values) / len(values)

        robomonkey_rate = None
        if "baseline_robomonkey" in consistency_map and -1 in consistency_map["baseline_robomonkey"]:
            values = consistency_map["baseline_robomonkey"][ -1 ]
            if values:
                robomonkey_rate = sum(values) / len(values)

        # Skip if no data at all
        if not series and oracle_rate is None and robomonkey_rate is None:
            continue

        # Determine figure width based on number of x points among all series
        max_points = 0
        for _, xs, _, _, _ in series:
            max_points = max(max_points, len(xs))
        plt.figure(figsize=(max(8, max_points * 1.2), 6))

        # Plot each series
        for label, xs, ys, ns, color in series:
            plt.plot(xs, ys, marker='o', color=color, label=f'{label}')
            # Annotate counts
            for x, y, n in zip(xs, ys, ns):
                plt.text(x, y + 0.03, f'n={n}', ha='center', fontsize=8, color=color)

        # Plot baselines if available
        if oracle_rate is not None:
            plt.axhline(y=oracle_rate, color='red', linestyle='--', alpha=0.7, label='oracle (no_transform)')
        if robomonkey_rate is not None:
            plt.axhline(y=robomonkey_rate, color='orange', linestyle='--', alpha=0.7, label='robomonkey (baseline)')

        # X ticks: unify across all series
        all_xs = sorted(set([x for _, xs, _, _, _ in series for x in xs]))
        if all_xs:
            plt.xticks(all_xs)
...
def create_task_consistency_time_series_plots(rollout_dir):
    """
    For each unique task (language instruction), create a time series plot showing
    consistency scores over time for different transformation types.
    X-axis: timestep
    ...
    # Aggregate: task -> transformation -> {success: [consistency_lists], failure: [consistency_lists]}
    task_transformation_consistency = defaultdict(lambda: defaultdict(lambda: {"success": [], "failure": []}))
    ...
    for trans in transformation_folders:
        ...
        for pkl_file in pkl_files:
            ...
            try:
                with open(pkl_file, "rb") as f:
                    data = pickle.load(f)
                    if "all_consistency_scores" not in data:
                        continue
                    consistency_scores = data["all_consistency_scores"]
                    
                    if consistency_scores is not None:
                        # Convert to numpy array and handle NaN values
                        consistency_np = np.array(consistency_scores, dtype=float)
                        if is_success:
                            task_transformation_consistency[task][trans]["success"].append(consistency_np)
                        else:
                            task_transformation_consistency[task][trans]["failure"].append(consistency_np)
            except Exception as e:
                print(f"Error loading {pkl_file}: {e}")
                continue
    
    # Create output directory
    task_consistency_dir = os.path.join(rollout_dir, "plots", "task_consistency_time_series")
    os.makedirs(task_consistency_dir, exist_ok=True)
    
    # Create a plot for each task
    for task, trans_dict in task_transformation_consistency.items():
        if not trans_dict:
            continue
        ...
        for i, trans in enumerate(sorted_transformations):
            group = trans_dict[trans]
            success_lists = group.get("success", [])
            failure_lists = group.get("failure", [])

            # Skip if no data at all
            if not success_lists and not failure_lists:
                continue

            # Find maximum length across all episodes for this transformation
            max_len_success = max((len(c) for c in success_lists), default=0)
            max_len_failure = max((len(c) for c in failure_lists), default=0)
            max_len = max(max_len_success, max_len_failure)

            # Calculate average consistency score at each timestep (success)
            avg_consistency_success = []
            std_consistency_success = []
            for t in range(max_len):
                timestep_vals = []
                for consistency in success_lists:
                    if t < len(consistency) and not np.isnan(consistency[t]):
                        timestep_vals.append(consistency[t])
                if timestep_vals:
                    avg_consistency_success.append(np.mean(timestep_vals))
                    std_consistency_success.append(np.std(timestep_vals))
                else:
                    avg_consistency_success.append(np.nan)
                    std_consistency_success.append(np.nan)

            # Calculate average consistency score at each timestep (failure)
            avg_consistency_failure = []
            std_consistency_failure = []
            for t in range(max_len):
                timestep_vals = []
                for consistency in failure_lists:
                    if t < len(consistency) and not np.isnan(consistency[t]):
                        timestep_vals.append(consistency[t])
                if timestep_vals:
                    avg_consistency_failure.append(np.mean(timestep_vals))
                    std_consistency_failure.append(np.std(timestep_vals))
                else:
                    avg_consistency_failure.append(np.nan)
                    std_consistency_failure.append(np.nan)

            timesteps = np.arange(1, max_len + 1)
            linestyle = '-' if trans in ["no_transform_1", "robomonkey"] else '--'
            linewidth = 1.5

            # Plot success line (green)
            valid_success = ~np.isnan(np.array(avg_consistency_success))
            if np.any(valid_success):
                plt.plot(timesteps[valid_success], np.array(avg_consistency_success)[valid_success],
                         color='green', linestyle=linestyle, linewidth=linewidth,
                         label=f'{trans} Success (n={len(success_lists)})')
                avg_array = np.array(avg_consistency_success)
                std_array = np.array(std_consistency_success)
                plt.fill_between(timesteps[valid_success],
                                 (avg_array - std_array)[valid_success],
                                 (avg_array + std_array)[valid_success],
                                 color='green', alpha=0.10)

            # Plot failure line (red)
            valid_failure = ~np.isnan(np.array(avg_consistency_failure))
            if np.any(valid_failure):
                plt.plot(timesteps[valid_failure], np.array(avg_consistency_failure)[valid_failure],
                         color='red', linestyle=linestyle, linewidth=linewidth,
                         label=f'{trans} Failure (n={len(failure_lists)})')
                avg_array = np.array(avg_consistency_failure)
                std_array = np.array(std_consistency_failure)
                plt.fill_between(timesteps[valid_failure],
                                 (avg_array - std_array)[valid_failure],
                                 (avg_array + std_array)[valid_failure],
                                 color='red', alpha=0.10)
        
        plt.xlabel('Timestep')
        plt.ylabel('Average Consistency Score')
        plt.title(f'Consistency Score Time Series for Task: {task.replace("_", " ")}')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True, alpha=0.2)
        plt.tight_layout()
        ...
        # Save the plot
        safe_task = re.sub(r'[^a-zA-Z0-9_\-]', '_', task)[:80]
        out_path = os.path.join(task_consistency_dir, f'{safe_task}_consistency_time_series.png')
        plt.savefig(out_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Saved consistency time series plot for task '{task}' to {out_path}")
...
def create_selected_phrases_bar_plots(rollout_dir):
    """
    Create bar plots showing the frequency of selected language instructions for each task and rephrase count,
    grouped by consistency=True/False. Each task-rephrase combination gets one figure with two bar plots side by side.
    X-axis shows the actual language instruction text.
    ...
    # Aggregate data: task -> rephrase_count -> consistency_flag -> Counter of instruction texts
    task_rephrase_consistency_instruction_counts = defaultdict(lambda: defaultdict(lambda: defaultdict(Counter)))
    
    for trans in transformation_folders:
        folder_path = os.path.join(rollout_dir, trans)
        pkl_files = glob(os.path.join(folder_path, "*.pkl"))
        
        # Determine rephrase count and consistency flag from folder name
        rephrase_match = re.search(r'rephrase_(\d+)', trans)
        if not rephrase_match:
            continue
        rephrase_count = int(rephrase_match.group(1))
        
        consistency_flag = None
        if "_consistency_True" in trans:
            consistency_flag = True
        elif "_consistency_False" in trans:
            consistency_flag = False
        # Skip folders without explicit consistency flag
        ...
        for pkl_file in pkl_files:
            ...
            try:
                with open(pkl_file, "rb") as f:
                    ...
                    # Extract task description list
                    task_description_list = data.get("task_description_list", [])
                    
                    if task_description_list:
                        # Count each instruction text
                        for instruction in task_description_list:
                            if instruction and isinstance(instruction, str):
                                task_rephrase_consistency_instruction_counts[task][rephrase_count][consistency_flag][instruction] += 1
                        
            except Exception as e:
                print(f"Error loading {pkl_file}: {e}")
                continue
    
    if not task_rephrase_consistency_instruction_counts:
        print("No instruction data found for selected phrases bar plots")
        return
    
    # Create output directory
    phrases_plot_dir = os.path.join(rollout_dir, "plots", "selected_phrases_analysis")
    os.makedirs(phrases_plot_dir, exist_ok=True)
    
    # Create a plot for each task and rephrase count combination
    for task in sorted(task_rephrase_consistency_instruction_counts.keys()):
        task_data = task_rephrase_consistency_instruction_counts[task]
        
        for rephrase_count in sorted(task_data.keys()):
            consistency_data = task_data[rephrase_count]
            
            # Get data for both consistency flags
            consistency_true_counts = consistency_data.get(True, Counter())
            consistency_false_counts = consistency_data.get(False, Counter())
            
            if not consistency_true_counts and not consistency_false_counts:
                continue
            
            # Get all unique instructions across both consistency flags
            all_instructions = set(consistency_true_counts.keys()) | set(consistency_false_counts.keys())
            ...
            for instruction in sorted_instructions:
                ...
                true_counts.append(consistency_true_counts.get(instruction, 0))
                false_counts.append(consistency_false_counts.get(instruction, 0))
            
            # Plot consistency=True data
            if consistency_true_counts:
                bars1 = ax1.bar(range(len(instructions)), true_counts, color='mediumseagreen', alpha=0.7, edgecolor='black')
                ax1.set_title(f'Consistency=True (Total: {sum(true_counts)})', fontsize=12, fontweight='bold')
                ax1.set_xlabel('Language Instructions')
                ax1.set_ylabel('Selection Count')
                ax1.grid(True, alpha=0.3, axis='y')
                
                # Set x-axis labels (instruction texts)
                ax1.set_xticks(range(len(instructions)))
                ax1.set_xticklabels(instructions, rotation=45, ha='right', fontsize=8)
                ...
                ax1.text(0.5, 0.5, 'No data for\nConsistency=True', 
                        ha='center', va='center', transform=ax1.transAxes, fontsize=12)
                ax1.set_title('Consistency=True (No data)', fontsize=12)
            
            # Plot consistency=False data
            if consistency_false_counts:
                bars2 = ax2.bar(range(len(instructions)), false_counts, color='royalblue', alpha=0.7, edgecolor='black')
                ax2.set_title(f'Consistency=False (Total: {sum(false_counts)})', fontsize=12, fontweight='bold')
                ax2.set_xlabel('Language Instructions')
                ax2.set_ylabel('Selection Count')
                ax2.grid(True, alpha=0.3, axis='y')
                
                # Set x-axis labels (instruction texts)
                ax2.set_xticks(range(len(instructions)))
                ax2.set_xticklabels(instructions, rotation=45, ha='right', fontsize=8)
                
                # Add count labels on bars
                for i, (bar, count) in enumerate(zip(bars2, false_counts)):
                    if count > 0:
                        height = bar.get_height()
                        ax2.text(bar.get_x() + bar.get_width()/2., height + 0.1,
                                f'{count}', ha='center', va='bottom', fontsize=8, fontweight='bold')
            else:
                ax2.text(0.5, 0.5, 'No data for\nConsistency=False', 
                        ha='center', va='center', transform=ax2.transAxes, fontsize=12)
                ax2.set_title('Consistency=False (No data)', fontsize=12)
            
            # Set common y-axis limits for better comparison
            max_count = max(max(true_counts) if true_counts else 0, max(false_counts) if false_counts else 0)
            if max_count > 0:
            ...
            # Overall title
            safe_task_name = task.replace('_', ' ')
            fig.suptitle(f'Selected Language Instructions: {safe_task_name} - {rephrase_count} Rephrases', fontsize=14, fontweight='bold')
            
            plt.tight_layout()
            
            # Save the plot
            safe_task = re.sub(r'[^a-zA-Z0-9_\-]', '_', task)[:80]
            out_path = os.path.join(phrases_plot_dir, f'{safe_task}_rephrase_{rephrase_count}_selected_instructions_analysis.png')
            plt.savefig(out_path, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"Saved selected instructions analysis for task '{task}' rephrase_{rephrase_count} to {out_path}")

if __name__ == "__main__":

    # path_to_rollouts_oracle = "./rollouts_clip_oracle"
    path_to_rollouts_clip = "./rollouts_clip_gaussian_consistency"
    ...
    # Call the new selected phrases bar plots function
    # create_selected_phrases_bar_plots(path_to_rollouts_clip)
