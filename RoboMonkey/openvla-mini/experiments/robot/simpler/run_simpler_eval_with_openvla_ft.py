...
def check_action_consistency_with_embeddings(model, candidate_instructions: List[str], 
                                            ...
                                            cfg: GenerateConfig) -> tuple[float, dict]:
    """
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task), desc="Trials", leave=False):
            ...
            while t < max_steps + cfg.num_steps_wait:
                ...
                consistency_info = {}
                if cfg.use_consistency_check and len(candidate_instructions) > 1:
                    ...
                    if len(valid_indices) > 1:
                        ...
                        # Get corresponding instructions and actions
                        top_instructions = []
                        top_actions = []
                        # Calculate actual actions per instruction
                        actual_actions_per_instruction = len(predicted_actions) // len(candidate_instructions)
                        # print(f"  Debug: {len(predicted_actions)} total actions, {len(candidate_instructions)} instructions, {actual_actions_per_instruction} actions per instruction")
                        
                        for idx in top_indices:
                            instruction_idx = idx // actual_actions_per_instruction
                            top_instructions.append(candidate_instructions[instruction_idx])
                            top_actions.append(predicted_actions[idx])
                        ...
                        # print(f"  [t={t}] Consistency: {consistency_score:.3f}")
                        # print(f"    Lipschitz constant: {consistency_info['lipschitz_constant']:.3f}, Mean ratio: {consistency_info['mean_lipschitz_ratio']:.3f}")
                        # print(f"    Correlation: {consistency_info['correlation']:.3f}, Stability: {1.0 - consistency_info['std_lipschitz_ratio']/max(consistency_info['mean_lipschitz_ratio'], 1e-6):.3f}")
                        # log_file.write(f"  [t={t}] Consistency: {consistency_score:.3f}\n")
                        # log_file.write(f"    Lipschitz constant: {consistency_info['lipschitz_constant']:.3f}, Mean ratio: {consistency_info['mean_lipschitz_ratio']:.3f}\n")
                        # log_file.write(f"    Correlation: {consistency_info['correlation']:.3f}, Stability: {1.0 - consistency_info['std_lipschitz_ratio']/max(consistency_info['mean_lipschitz_ratio'], 1e-6):.3f}\n")
                    else:
                ...
                if cfg.clip_select_action_num_candidates > 1:
                    # Calculate actual actions per instruction for mapping
                    actual_actions_per_instruction = len(predicted_actions) // len(candidate_instructions)
                    if cfg.clip_select_action_strategy == "highest_score":
                        ...
                        if len(valid_indices) == 0:
                            ...
                            # Map action index back to instruction index
                            instruction_idx = best_candidate_idx // actual_actions_per_instruction
                            all_selected_instructions.append(candidate_instructions[instruction_idx])
