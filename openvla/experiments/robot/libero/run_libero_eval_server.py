...
class GenerateConfig:
    ...
    # --- Policy Server (Batch Inference) ---
    use_policy_server: bool = True                      # Enable external policy server
    policy_server_url: str = "http://localhost:3200"     # Policy server URL
    policy_temperature: float = 0.2                      # Sampling temperature for server
...
def get_batch_actions(instructions, image_path, server_url, temperature=1.0):
    """Request batched actions for multiple language instructions from policy server."""
    ...
    try:
        ...
        # Expect keys: output_ids, actions
        return np.array(result.get("output_ids")), np.array(result.get("actions"))
    except Exception as e:
        print(f"Error calling policy server: {e}")
        return None, None
...
def eval_libero(cfg: GenerateConfig) -> None:
    ...
    if cfg.use_policy_server:
        print(f"Using Policy Server: {cfg.policy_server_url}")
    else:
        print("Policy Server DISABLED.")
    ...
    for task_id in tqdm(range(num_tasks_in_suite), desc="Tasks"):
        ...
        for episode_idx in tqdm(range(cfg.num_trials_per_task), desc="Trials", leave=False):
            ...
            while t < max_steps:
                ...
                # Save image for policy server if needed
                server_image_path = None
                if cfg.use_policy_server:
                    server_image_path = os.path.abspath("./transfer_images/libero_img.jpg")
                    os.makedirs(os.path.dirname(server_image_path), exist_ok=True)
                    try:
                        imageio.imwrite(server_image_path, img_for_vla)
                    except Exception as e:
                        print(f"Warning: failed to save image for policy server: {e}")

                # 1. Generate action for the original instruction (via server or local model)
                if cfg.use_policy_server:
                    _, actions_batch = get_batch_actions([task_description], server_image_path, cfg.policy_server_url, cfg.policy_temperature)
                    if actions_batch is None or len(actions_batch) == 0:
                        # Fallback to local model
                        original_action = get_action(cfg, model, observation, task_description, processor=processor)
                    else:
                        original_action = actions_batch[0]
                else:
                    original_action = get_action(cfg, model, observation, task_description, processor=processor)
                ...
                if (vla_clip_scorer and
                    ...
                    if num_to_generate > 0:
                        ...
                        # Predict actions for additional candidate instructions (server or local)
                        predicted_actions = [original_action]
                        padded_histories = [original_padded_history]
                        actions_batch = None
                        if cfg.use_policy_server:
                            _, actions_batch = get_batch_actions(additional_instructions, server_image_path, cfg.policy_server_url, cfg.policy_temperature)
                            if actions_batch is None or len(actions_batch) != len(additional_instructions):
                                print("Warning: policy server failed, falling back to local model for candidates.")
                                actions_batch = None
                        if actions_batch is not None:
                            for a in actions_batch:
                                a_norm = normalize_gripper_action(a, binarize=True)
                                if cfg.model_family == "openvla":
                                    a_norm = invert_gripper_action(a_norm)
                                predicted_actions.append(a_norm)
                        else:
                        ...
                        # Build padded histories aligned with predicted actions
                        for a in predicted_actions[1:]:
                            hist_list = list(executed_action_history)
                            H = cfg.vla_clip_history_length
                            num_pad = H - len(hist_list) - 1
                            past = [padding_action_vector] * max(0, num_pad) + hist_list[-(H - 1):]
