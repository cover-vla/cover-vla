...
def get_libero_env(task, model_family, resolution=256, task_seed=0):
...
def get_libero_image(obs, resize_size=None, key='agentview_image'):
...
def save_rollout_video(rollout_images, idx, success, transform_type,
    ...
    if oracle_scorer:
        rollout_dir = f"./rollouts_clip_oracle/{transform_type}_{clip_update_num}"
    else:
        rollout_dir = f"./rollouts_clip/{transform_type}_{clip_update_num}"
