...
# Import our standalone VQ-VAE implementation
from action_vqvae.action_vqvae_wrapper import ActionVQVAELossWrapper

"""
VQ-VAE Action Tokenizer Integration with VQ-VLA Pre-trained Weights

This script now uses a standalone VQ-VAE implementation based on VQ-VLA's architecture
for proper integration with their pre-trained weights.

The VQ-VAE architecture from VQ-VLA:
- Encoder: Causal 2D convolutional layers (1->128->256->256->512 channels)
- Latent dimension: 128
- Vector quantization: ResidualVQ with 4 groups, 256 codebook size
- Decoder: Causal 2D deconvolutional layers (512->256->256->128->1)

This implementation will properly leverage VQ-VLA's pre-trained action representations
trained on 100x more data than previous approaches.
"""
...
def train_clip_vqvae(
    ...
    if use_wandb:
        ...
        if save_name is None:
             save_name = f"vla_clip_vqvae_h{history_length}"
             print(f"Generated save_name for wandb: {save_name}")

        wandb.init(project="VLA-CLIP-VQVAE", name=save_name)
        wandb.config.update({
            ...
            "vqvae_checkpoint": vqvae_checkpoint_path,
    ...
    if save_name is None:
         save_name = f"vla_clip_vqvae_h{history_length}_local"
    ...
    # Initialize the model with VQ-VAE action tokenizer
    model = VLA_CLIP_VQVAE(model_config, vqvae_checkpoint_path, device).float().to(device)
...
class VLA_CLIP_VQVAE(nn.Module):
    def __init__(self, model_config, vqvae_checkpoint_path, device="cuda"):
        super().__init__()
        ...
        self.text_pooling = AttentionPooling(
            text_dim, text_pooling_output_dim, pooling_heads, pooling_layers, num_readouts=self.num_readouts
        )        
        self.agentview_vision_poolings = AttentionPooling(
            vision_dim, vision_pooling_output_dim, pooling_heads, pooling_layers, num_readouts=self.num_readouts
        )
        ...
        # Action trajectory processing using VQ-VAE
        self.action_dim = model_config.action_dim
        self.history_length = model_config.history_length
        
        # Initialize VQ-VLA's real VQ-VAE action tokenizer
        # The wrapper expects: model_path (directory with config), checkpoint_path (weights file)
        # The checkpoint was trained with use_action_type_pe=True and use_time_pe=True
        config_dir = os.path.dirname(vqvae_checkpoint_path).replace('action_tokenizer_weight', 'action_vqvae_config')
        self.vqvae_action_tokenizer = ActionVQVAELossWrapper(
            model_path=config_dir,
            checkpoint_path=vqvae_checkpoint_path,
            use_action_type_pe=True,  # Enable action type embeddings (xyz, euler, gripper)
            use_time_pe=True,         # Enable time embeddings
            freeze=True,               # Freeze the VQ-VAE for inference
            is_eval=True               # Set to evaluation mode
        )
        
        # Project VQ-VAE output to match vision dimension
        # VQ-VLA's real VQ-VAE outputs 128-dimensional latents (from config: encoder_out_channels: 128)
        self.action_projection = nn.Linear(128, vision_pooling_output_dim)
    ...
    def extract_clip_features(self, images, text):
        ...
        patch_features = patch_features.permute(1, 0, 2)[:, 1:, :]
    ...
    def forward(self, image1, image2, text, action_histories):
        ...
        # Encode Action History using VQ-VAE
        action_histories = action_histories.float().to(image1.device)
        
        # Use VQ-VLA's real VQ-VAE action tokenizer
        encoded_actions = self.vqvae_action_tokenizer.get_embeddings(action_histories)
        
        # Project to match vision dimension
        projected_trajectory = self.action_projection(encoded_actions)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train VLA-CLIP model with VQ-VAE action tokenizer and contrastive loss')
    # Training parameters
    ...
    parser.add_argument('--vqvae_checkpoint', type=str, required=True, help='Path to VQ-VAE action tokenizer checkpoint')

    # Dataset and paths
    parser.add_argument('--augmented_dataset', type=str, required=True, help='Path to augmented dataset pickle file (with histories)')
    parser.add_argument('--checkpoint_dir', type=str, default='vqvae_checkpoints', help='Directory to save checkpoints')
    ...
    if args.save_name is None:
         args.save_name = f"vla_clip_vqvae_h{args.history_length}"
    FINAL_SAVE_PATH = os.path.join(args.checkpoint_dir, f"{args.save_name}_final_best.pt")

    print("Starting training...")
    print(f"Config: History={args.history_length}, VQ-VAE={args.vqvae_checkpoint}, LR={args.lr}, BS={args.batch_size}")
    print(f"Using wandb: {args.use_wandb}")
    if args.resume:
        print(f"Resuming from checkpoint: {args.resume}")

    finetuned_model = train_clip_vqvae(
