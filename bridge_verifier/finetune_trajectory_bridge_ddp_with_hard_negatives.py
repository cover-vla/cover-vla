...
class BridgeDatasetWithHardNegatives(Dataset):
    def __init__(self, augmented_dataset_dict, history_length, images_folder):
        """
        Memory-efficient Dataset for Bridge V2 with hard negatives
    ...
    def _load_hard_negatives_format_efficient(self, augmented_dataset_dict):
        """Memory-efficient loading with hard negatives"""
        ...
        print(f"{valid_count:,} valid samples processed")
    
    def _load_normalized_format_efficient(self, augmented_dataset_dict):
        """Memory-efficient loading (fallback for older format)"""
        # Store references to lookup tables
        self.action_histories = augmented_dataset_dict['action_histories']
        self.instructions = augmented_dataset_dict['instructions']
        samples_data = augmented_dataset_dict['samples']
    ...
    def _load_legacy_format_efficient(self, augmented_dataset_dict):
        """Memory-efficient legacy format loading"""
        # For legacy format, we need to flatten the samples
        all_samples = []
        
        print("Collecting legacy format samples...")
        ...
        # Store all samples (DistributedSampler will handle sharding)
        total_samples = len(all_samples)
        print(f"Collected {total_samples:,} legacy format samples")
        
        # Store all samples
        self.sample_indices = all_samples
...
def weighted_infonce_loss_with_hard_negatives(
    ...
    disable_hard_negatives=False  # If True, only use in-batch negatives
):
    """
    ...
    for i in range(batch_size):
        ...
        # Add hard negatives if available and not disabled
        if not disable_hard_negatives and i < len(hard_negative_features) and hard_negative_features[i].shape[0] > 0:
...
def train_clip_bridge_hard_negatives_ddp(
    ...
    use_bf16 = None,
    cosine_cycles = 0.5,
    hard_negative_micro_batch_size = 512,
    ...
    # Load dataset efficiently for large datasets with DistributedSampler
    if rank == 0:
        print(f"Loading dataset from {dataset_path}...")
        print("Note: Using DistributedSampler for efficient memory usage and proper data distribution")
    ...
    try:
        ...
        # Create dataset (DistributedSampler will handle sharding)
        ...
        if rank == 0 and torch.cuda.is_available():
            ...
            print(f"DistributedSampler will distribute {len(dataset):,} samples across {world_size} GPUs")
    ...
    if rank == 0:
        ...
        print(f"Total effective batch size across {world_size} GPUs: {batch_size * world_size}")
        print(f"Samples per GPU (approx): Train={len(train_dataset) // world_size}, Val={len(val_dataset) // world_size}")

    # Create DistributedSamplers for proper DDP data distribution
    train_sampler = DistributedSampler(
        ...
        drop_last=False  # Don't drop incomplete batches to use all data
    )
    ...
    val_dataloader = DataLoader(
        val_dataset, 
        batch_size=batch_size,
        sampler=val_sampler,  # Use DistributedSampler
    ...
    for epoch in epoch_pbar:
        ...
        total_img2act_top1_acc = 0
        total_img2act_top5_acc = 0
        total_act2img_top1_acc = 0
        total_act2img_top5_acc = 0
        ...
        for batch_idx, batch_data in enumerate(train_batch_pbar):
            ...
            # Prepare hard negatives for weighted InfoNCE (MICRO-BATCHED VERSION)
            hard_negative_features_list = []
            hard_negative_weights_list = []
            
            if not disable_hard_negatives:
                # Use the configurable micro batch size parameter
                
                # 1. Gather all hard negative texts and their source data
                all_hn_texts = []
                ...
                # 2. Run forward passes for all hard negatives in micro-batches
                if all_hn_texts:
                    all_hn_features_collected = []
                    num_hard_negatives = len(all_hn_texts)
                    
                    for start_idx in range(0, num_hard_negatives, hard_negative_micro_batch_size):
                        end_idx = min(start_idx + hard_negative_micro_batch_size, num_hard_negatives)
                        
                        # Slice the data for the current micro-batch
                        hn_texts_micro_batch = all_hn_texts[start_idx:end_idx]
                        hn_indices_micro_batch = hn_source_indices[start_idx:end_idx]
                        
                        # Tokenize only the current micro-batch of texts
                        tokenized_hn_micro_batch = clip.tokenize(hn_texts_micro_batch, truncate=True).to(device)
                        
                        # Create small tensors for images and actions for this micro-batch
                        images_for_hn_micro_batch = images[hn_indices_micro_batch]
                        actions_for_hn_micro_batch = action_histories[hn_indices_micro_batch]
                        
                        # Run the forward pass for the micro-batch
                        with torch.no_grad(), torch.amp.autocast('cuda', enabled=use_bf16, dtype=torch.bfloat16):
                            hn_features_micro_batch, _ = model(images_for_hn_micro_batch, tokenized_hn_micro_batch, actions_for_hn_micro_batch)
                        
                        all_hn_features_collected.append(hn_features_micro_batch)
                    
                    # Concatenate features from all micro-batches
                    all_hn_features = torch.cat(all_hn_features_collected, dim=0)
                    
                    # 3. Distribute features back to their original positive samples
                    current_pos = 0
                    for i in range(total_positives):
                ...
                # Hard negatives disabled - create empty lists
                for i in range(total_positives):
                    hard_negative_features_list.append(torch.empty(0, combined_features.shape[1], device=device))
                    hard_negative_weights_list.append(torch.empty(0, device=device))
            ...
            # Create logits for retrieval accuracy calculation
            logits_per_image = logit_scale * torch.matmul(combined_features, action_features.T)  # (total_positives, total_positives)
            logits_per_action = logits_per_image.T  # (total_positives, total_positives)
            
            retrieval_metrics = calculate_accuracy_metrics(
                logits_per_image, logits_per_action, total_positives, device, k_values=[1, 5]
            )
            ...
            total_img2act_top1_acc += retrieval_metrics.get('img2act_top1_acc', 0)
            total_img2act_top5_acc += retrieval_metrics.get('img2act_top5_acc', 0)
            total_act2img_top1_acc += retrieval_metrics.get('act2img_top1_acc', 0)
            total_act2img_top5_acc += retrieval_metrics.get('act2img_top5_acc', 0)
            train_batch_count += 1
            ...
            if global_step % train_log_freq == 0:
                ...
                step_metrics = torch.tensor([
                    ...
                    retrieval_metrics.get('img2act_top1_acc', 0),
                    retrieval_metrics.get('img2act_top5_acc', 0),
                    retrieval_metrics.get('act2img_top1_acc', 0),
                    retrieval_metrics.get('act2img_top5_acc', 0)
                ], device=device)
                ...
                avg_img2act_top1_acc = step_metrics[6].item()
                avg_img2act_top5_acc = step_metrics[7].item()
                avg_act2img_top1_acc = step_metrics[8].item()
                avg_act2img_top5_acc = step_metrics[9].item()
                ...
                if rank == 0:
                    ...
                    log_dict = {
                        ...
                        "train/step_img2act_top1_acc": avg_img2act_top1_acc,
                        "train/step_img2act_top5_acc": avg_img2act_top5_acc,
                        "train/step_act2img_top1_acc": avg_act2img_top1_acc,
                        "train/step_act2img_top5_acc": avg_act2img_top5_acc,
                    }
                    ...
                    print(f"Step {global_step}: Loss={avg_loss_item:.4f}, LR={current_lr:.2e}, "
                          f"HardNegAcc={avg_accuracy:.3f}, Img2Act@1={avg_img2act_top1_acc:.3f}")
            ...
            if global_step % eval_log_freq == 0 and global_step > 0:
                ...
                eval_losses = []
                eval_hard_neg_accs = []
                eval_img2act_top1_accs = []
                eval_img2act_top5_accs = []
                eval_act2img_top1_accs = []
                eval_act2img_top5_accs = []
                    ...
                    for eval_batch_idx, eval_batch_data in enumerate(val_dataloader):
                        ...
                        if not disable_hard_negatives:
                            ...
                            if eval_all_hn_texts:
                                eval_all_hn_features_collected = []
                                eval_num_hard_negatives = len(eval_all_hn_texts)
                                
                                for start_idx in range(0, eval_num_hard_negatives, hard_negative_micro_batch_size):
                                    end_idx = min(start_idx + hard_negative_micro_batch_size, eval_num_hard_negatives)
                                    
                                    # Slice the data for the current micro-batch
                                    eval_hn_texts_micro_batch = eval_all_hn_texts[start_idx:end_idx]
                                    eval_hn_indices_micro_batch = eval_hn_source_indices[start_idx:end_idx]
                                    
                                    # Tokenize only the current micro-batch of texts
                                    tokenized_eval_hn_micro_batch = clip.tokenize(eval_hn_texts_micro_batch, truncate=True).to(device)
                                    
                                    # Create small tensors for images and actions for this micro-batch
                                    eval_images_for_hn_micro_batch = eval_images[eval_hn_indices_micro_batch]
                                    eval_actions_for_hn_micro_batch = eval_action_histories[eval_hn_indices_micro_batch]
                                    
                                    # Run the forward pass for the micro-batch
                                    with torch.no_grad(), torch.amp.autocast('cuda', enabled=use_bf16, dtype=torch.bfloat16):
                                        eval_hn_features_micro_batch, _ = model(eval_images_for_hn_micro_batch, tokenized_eval_hn_micro_batch, eval_actions_for_hn_micro_batch)
                                    
                                    eval_all_hn_features_collected.append(eval_hn_features_micro_batch)
                                
                                # Concatenate features from all micro-batches
                                eval_all_hn_features = torch.cat(eval_all_hn_features_collected, dim=0)
                        ...
                        # Calculate retrieval accuracy metrics for step-wise evaluation
                        eval_step_total_positives = len(eval_positive_instructions)
                        eval_step_logits_per_image = logit_scale * torch.matmul(eval_combined_features, eval_action_features.T)
                        eval_step_logits_per_action = eval_step_logits_per_image.T
                        
                        eval_step_retrieval_metrics = calculate_accuracy_metrics(
                            eval_step_logits_per_image, eval_step_logits_per_action, eval_step_total_positives, device, k_values=[1, 5]
                        )
                        
                        eval_losses.append(eval_loss.item())
                        eval_hard_neg_accs.append(eval_loss_metrics['accuracy'])
                        eval_img2act_top1_accs.append(eval_step_retrieval_metrics.get('img2act_top1_acc', 0))
                        eval_img2act_top5_accs.append(eval_step_retrieval_metrics.get('img2act_top5_acc', 0))
                        eval_act2img_top1_accs.append(eval_step_retrieval_metrics.get('act2img_top1_acc', 0))
                        eval_act2img_top5_accs.append(eval_step_retrieval_metrics.get('act2img_top5_acc', 0))
                ...
                avg_eval_img2act_top1_acc = np.mean(eval_img2act_top1_accs) if eval_img2act_top1_accs else 0
                avg_eval_img2act_top5_acc = np.mean(eval_img2act_top5_accs) if eval_img2act_top5_accs else 0
                avg_eval_act2img_top1_acc = np.mean(eval_act2img_top1_accs) if eval_act2img_top1_accs else 0
                avg_eval_act2img_top5_acc = np.mean(eval_act2img_top5_accs) if eval_act2img_top5_accs else 0
                ...
                if rank == 0:
                    eval_log_dict = {
                        ...
                        "eval/step_img2act_top1_acc": avg_eval_img2act_top1_acc,
                        "eval/step_img2act_top5_acc": avg_eval_img2act_top5_acc,
                        "eval/step_act2img_top1_acc": avg_eval_act2img_top1_acc,
                        "eval/step_act2img_top5_acc": avg_eval_act2img_top5_acc,
                    ...
                    print(f"Eval at step {global_step}: Loss={avg_eval_loss:.4f}, "
                          f"HardNegAcc={avg_eval_hard_neg_acc:.3f}, Img2Act@1={avg_eval_img2act_top1_acc:.3f}")
        ...
        metrics_to_sync = torch.tensor([
            ...
            total_img2act_top1_acc, total_img2act_top5_acc,
            total_act2img_top1_acc, total_act2img_top5_acc,
            train_batch_count
        ], device=device)
        ...
        avg_train_img2act_top1_acc = total_img2act_top1_acc / train_batch_count
        avg_train_img2act_top5_acc = total_img2act_top5_acc / train_batch_count
        avg_train_act2img_top1_acc = total_act2img_top1_acc / train_batch_count
        avg_train_act2img_top5_acc = total_act2img_top5_acc / train_batch_count
        ...
        total_val_img2act_top1_acc = 0
        total_val_img2act_top5_acc = 0
        total_val_act2img_top1_acc = 0
        total_val_act2img_top5_acc = 0
        val_batch_count = 0
        ...
        with torch.no_grad():
            for batch_idx, batch_data in enumerate(val_batch_pbar):
                ...
                if not disable_hard_negatives:
                    ...
                    # 2. Run forward passes for all hard negatives in micro-batches
                    if val_all_hn_texts:
                        val_all_hn_features_collected = []
                        val_num_hard_negatives = len(val_all_hn_texts)
                        
                        for start_idx in range(0, val_num_hard_negatives, hard_negative_micro_batch_size):
                            end_idx = min(start_idx + hard_negative_micro_batch_size, val_num_hard_negatives)
                            
                            # Slice the data for the current micro-batch
                            val_hn_texts_micro_batch = val_all_hn_texts[start_idx:end_idx]
                            val_hn_indices_micro_batch = val_hn_source_indices[start_idx:end_idx]
                            
                            # Tokenize only the current micro-batch of texts
                            tokenized_val_hn_micro_batch = clip.tokenize(val_hn_texts_micro_batch, truncate=True).to(device)
                            
                            # Create small tensors for images and actions for this micro-batch
                            val_images_for_hn_micro_batch = images[val_hn_indices_micro_batch]
                            val_actions_for_hn_micro_batch = action_histories[val_hn_indices_micro_batch]
                            
                            # Run the forward pass for the micro-batch
                            with torch.no_grad(), torch.amp.autocast('cuda', enabled=use_bf16, dtype=torch.bfloat16):
                                val_hn_features_micro_batch, _ = model(val_images_for_hn_micro_batch, tokenized_val_hn_micro_batch, val_actions_for_hn_micro_batch)
                            
                            val_all_hn_features_collected.append(val_hn_features_micro_batch)
                        
                        # Concatenate features from all micro-batches
                        val_all_hn_features = torch.cat(val_all_hn_features_collected, dim=0)
                ...
                # Calculate retrieval accuracy metrics for validation
                val_total_positives = len(positive_instructions)
                val_logits_per_image = logit_scale * torch.matmul(combined_features, action_features.T)  # (val_total_positives, val_total_positives)
                val_logits_per_action = val_logits_per_image.T  # (val_total_positives, val_total_positives)
                
                val_retrieval_metrics = calculate_accuracy_metrics(
                    val_logits_per_image, val_logits_per_action, val_total_positives, device, k_values=[1, 5]
                )
                ...
                total_val_img2act_top1_acc += val_retrieval_metrics.get('img2act_top1_acc', 0)
                total_val_img2act_top5_acc += val_retrieval_metrics.get('img2act_top5_acc', 0)
                total_val_act2img_top1_acc += val_retrieval_metrics.get('act2img_top1_acc', 0)
                total_val_act2img_top5_acc += val_retrieval_metrics.get('act2img_top5_acc', 0)
                val_batch_count += 1
        ...
        val_metrics_to_sync = torch.tensor([
            ...
            total_val_img2act_top1_acc, total_val_img2act_top5_acc,
            total_val_act2img_top1_acc, total_val_act2img_top5_acc,
            val_batch_count
        ], device=device)
        ...
        avg_val_img2act_top1_acc = total_val_img2act_top1_acc / val_batch_count
        avg_val_img2act_top5_acc = total_val_img2act_top5_acc / val_batch_count
        avg_val_act2img_top1_acc = total_val_act2img_top1_acc / val_batch_count
        avg_val_act2img_top5_acc = total_val_act2img_top5_acc / val_batch_count
        ...
        if rank == 0:
            ...
            if use_wandb:
                ...
                log_dict = {
                    ...
                    "train/img2act_top1_acc": avg_train_img2act_top1_acc,
                    "train/img2act_top5_acc": avg_train_img2act_top5_acc,
                    "train/act2img_top1_acc": avg_train_act2img_top1_acc,
                    "train/act2img_top5_acc": avg_train_act2img_top5_acc,
                    
                    # Validation metrics
                    "val/loss": avg_val_loss,
                    "val/weighted_infonce_loss": avg_val_weighted_infonce_loss,
                    "val/hard_neg_accuracy": avg_val_hard_neg_accuracy,
                    "val/img2act_top1_acc": avg_val_img2act_top1_acc,
                    "val/img2act_top5_acc": avg_val_img2act_top5_acc,
                    "val/act2img_top1_acc": avg_val_act2img_top1_acc,
                    "val/act2img_top5_acc": avg_val_act2img_top5_acc,
            ...
            if epoch % 10 == 0:
                ...
                print(f"  Training   - Img2Act@1: {avg_train_img2act_top1_acc:.3f}, Img2Act@5: {avg_train_img2act_top5_acc:.3f}")
                print(f"  Training   - Act2Img@1: {avg_train_act2img_top1_acc:.3f}, Act2Img@5: {avg_train_act2img_top5_acc:.3f}")
                print(f"  Validation - Loss: {avg_val_loss:.4f}, Hard Neg Acc: {avg_val_hard_neg_accuracy:.3f}")
                print(f"  Validation - Img2Act@1: {avg_val_img2act_top1_acc:.3f}, Img2Act@5: {avg_val_img2act_top5_acc:.3f}")
                print(f"  Validation - Act2Img@1: {avg_val_act2img_top1_acc:.3f}, Act2Img@5: {avg_val_act2img_top5_acc:.3f}")
...
def ddp_main(rank, world_size, args):
    ...
    model = train_clip_bridge_hard_negatives_ddp(
        ...
        hard_negative_micro_batch_size=args.hard_negative_micro_batch_size,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hard_negative_micro_batch_size', type=int, default=512, help='Micro-batch size for processing hard negatives to avoid OOM')
    parser.add_argument('--disable_hard_negatives', action='store_true', help='Disable hard negative processing and use only in-batch negatives')
    ...
    print(f"DDP Config: World Size={args.world_size}, Port={args.port}, Using DistributedSampler")
    print(f"Hard Negatives Config: {'DISABLED (In-Batch Only)' if args.disable_hard_negatives else f'Alpha={args.hard_negative_alpha}, Micro Batch Size={args.hard_negative_micro_batch_size}'}")
    ...
    print("Note: Large dataset memory optimization enabled with DistributedSampler")
