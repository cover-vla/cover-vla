...
def get_text_embedding_similarity(rollout_path, save_plot=False):
    ...
    rollout_dir = rollout_path.split("/")
    rollout_dir[-1] = "no_transform"
    untransformed_path = "/".join(rollout_dir)

    text_embedding_untransformed_path = os.path.join(untransformed_path, "text_embeddings")
    text_embedding_transformed_path = os.path.join(rollout_path, "text_embeddings")
    ...
    for text_embedding in tqdm.tqdm(os.listdir(text_embedding_untransformed_path)):
        ...
        short_text_embedding = "--" + short_text_embedding[1] + "--"
...
def get_action_similarity(rollout_path, save_plot=False):
    ...
    action_transformed_path = os.path.join(rollout_path, "actions")
...
def get_entropy(rollout_path, save_plot=False, transformation="untransformed"):
    ...
    action_transformed_path = os.path.join(rollout_path, "action_probs")
...
def main():
    ...
    # transformation_type_list = ["synonym", "antonym","negation", "verb_noun_shuffle", "in_set", "out_set", "random_shuffle", "interpolation"]
    transformation_type_list = ["interpolation", "random_shuffle"]
