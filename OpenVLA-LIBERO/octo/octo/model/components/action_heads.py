...
class ActionHead(ABC):
    ...
    #     **kwargs,
    # ) -> jax.Array:
    #     """Predict the entropy of the action distribution for the last timestep in the window. Returns shape
    #     (*sample_shape, batch_size, pred_horizon, action_dim).
    #     """
    #     raise NotImplementedError
...
class DiffusionActionHead(nn.Module):
    ...
    def __call__(
        ...
        pred_eps, hidden_representation = self.diffusion_model(embeddings, noisy_actions, time, train=train)
        return pred_eps, hidden_representation
    ...
    def predict_action(
        ...
        **kwargs,
        ...
        def scan_fn(carry, time):
            current_x, rng, _ = carry
            input_time = jnp.broadcast_to(time, (*current_x.shape[:-1], 1))

            eps_pred, hidden_rep = module.apply(
            ...
            return (current_x, rng, hidden_rep), ()
        ...
        def sample_actions(rng):
            ...
            initial_hidden_rep = jnp.zeros((batch_size, window_size, self.hidden_dim))  

            (actions_flat, _, hidden_rep), () = jax.lax.scan(
                scan_fn,
                (
                    jax.random.normal(
                        key,
            ...
            # only get the last timestep in the window
            return actions[:, -1], hidden_rep

        n_samples = int(np.prod(sample_shape))
        actions, hidden_states = jax.vmap(sample_actions)(jax.random.split(rng, n_samples))
    ...
    #     def estimate_action_entropy(actions: jax.Array) -> jax.Array:
    #         """Estimates differential entropy using sample variance.
            
    #         Args:
    #             actions: Shape (n_samples, batch_size, pred_horizon, action_dim)
                
    #         Returns:
    #             entropy: Shape (batch_size, pred_horizon)
    #         """
    #         # Compute variance across samples (axis 0)
    #         action_variance = jnp.var(actions, axis=0)
            
    #         # Differential entropy for each action dimension (assuming Gaussian)
    #         entropy_per_dim = 0.5 * jnp.log(2 * jnp.pi * jnp.e * action_variance)
            
    #         # Sum over action dimensions
    #         total_entropy = jnp.sum(entropy_per_dim, axis=-1)
    #         return total_entropy
        
    #     actions = self.predict_action(transformer_outputs, rng, train=train, sample_shape=sample_shape)
    #     # Calculate entropy: shape (batch_size, pred_horizon, 7)
    #     entropy = estimate_action_entropy(actions)
        
    #     # Average over samples and timesteps: shape (7,)
    #     avg_entropy_per_dim = jnp.mean(entropy, axis=(0, 1))
        
    #     # If you want a single scalar value (average over all dimensions)
    #     final_entropy = jnp.mean(avg_entropy_per_dim)
        
    #     return final_entropy  # or return avg_entropy_per_dim for per-dimension values