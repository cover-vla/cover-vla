...
for task_suite_name in libero_task_map.keys():
    ...
    for task in list_of_tasks:
        print('task', task)
        cleaned_task = remove_scene_prefix(task)
        print('cleaned_task', cleaned_task)
        set_of_unique_words.update(cleaned_task.split('_'))
        print('-------------')
...
with open("unique_words.json", "w") as json_file:
    json.dump(list(set_of_unique_words), json_file)

print('saved unique words to unique_words.json')

with open("unique_words.json", "r") as json_file:
    loaded_words = set(json.load(json_file))
