...
for task in loaded_dic.keys():
    for transform in loaded_dic[task].keys():
        if transform == 'synonym_noun':  # Only process synonym_noun transformations
            for i in range(len(loaded_dic[task][transform])):
                if loaded_dic[task][transform][i] is None:
                    loaded_dic[task][transform][i] = lang_transform.transform(task, transform)
                    print('original: ', task)
                    print('transformed: ', loaded_dic[task][transform][i])
# Save the updated dictionary back to the file
with open('./negative_set.json', 'w') as f:
    json.dump(loaded_dic, f)
